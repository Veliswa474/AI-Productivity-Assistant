...
CREATE TABLE public.profiles (
  ...
  ai_creativity NUMERIC NOT NULL DEFAULT 0.7,
...
-- Generations (history for email, notes, research, planner)
CREATE TABLE public.generations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  type TEXT NOT NULL CHECK (type IN ('email','notes','research','planner')),
  title TEXT NOT NULL,
...
CREATE POLICY "own gens" ON public.generations FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
